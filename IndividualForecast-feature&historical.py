...
pred_f = MLP_f(_X_f, _Dropout_f, weights_f, biases_f)
...
pred_h = MLP_h(_X_h,  weights_h, biases_h)
reshaped_results_h = tf.reshape(_Y_h, [-1])
reshaped_outputs_h = tf.reshape(pred_h, [-1])
cost_h = tf.reduce_mean(tf.pow(reshaped_results_h - reshaped_outputs_h,2))
...
#n_hidden_s3_3 = 30
#n_hidden_s3_4 = 30
_A = tf.placeholder(tf.float32, [test_batch_size_h, s3_input])
_B = tf.placeholder(tf.float32, [test_batch_size_h, s3_input])
_I = tf.concat(1, [_A,_B])
_O = tf.placeholder(tf.float32, [test_batch_size_h, s3_input])
weights_s3 = {
...
for i in range(n_epochs):
    ...
    p_f = sess1.run(pred_f, feed_dict = {_X_f: x_f, _Y_f: y_f})
    p_h = sess2.run(pred_h, feed_dict = {_X_h: x_h, _Y_h: y_h})
    p_f = (np.array(p_f)).reshape([test_batch_size_h, s3_input])
    p_h = (np.array(p_h)).reshape([test_batch_size_h, s3_input])
    y_s3 = y_h
    _, err = sess3.run([optimizer_s3, cost_s3], feed_dict = {_A: p_f, _B: p_h, _O: y_s3})
    if i % 100 == 0:
        ...
        p_h = (np.array(p_h)).reshape([test_batch_size_h, s3_input])
        y_s3 = y_h
        P, err_t = sess3.run([_P, cost_s3], feed_dict = {_A: p_f, _B: p_h, _O: y_s3})
        ...
        predlist.append(P)
        predlistA.append(np.reshape(np.array(p_f), [-1]))
        predlistB.append(np.reshape(np.array(p_h), [-1]))
...
for prload in predlistA:
    prload = np.reshape(np.array(prload), [-1])
    rmseListA.append(rmse(prload,acload))
    mapeListA.append(mape(prload,acload))
for prload in predlistB:
    prload = np.reshape(np.array(prload), [-1])
    rmseListB.append(rmse(prload,acload))
    mapeListB.append(mape(prload,acload))

print "Feature-based-model MAPE = " + str(np.mean(mapeListA[-101:-1]))
print "Historical-load-based model MAPE = " + str(np.mean(mapeListB[-101:-1]))
